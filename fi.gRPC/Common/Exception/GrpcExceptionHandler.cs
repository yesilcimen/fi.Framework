    ...
    public static class GrpcExceptionHandler
    {
        public static bool SetException(Metadata metadata, Exception exception)
        {
            ...
            if (exceptionSerializer != null)
            {
                ...
                metadata.Add(new Metadata.Entry(Constants.GrpcMetaDataKeyError, serializedExceptionContent));
        ...
        public static bool TryGetErrorModel(Exception exception, out ErrorModel errorModel)
        {
            ...
            {
                if (rpcException.Trailers != null && string.Equals(rpcException.Status.Detail, Constants.GrpcMetaDataKeyError, StringComparison.OrdinalIgnoreCase))
                {
        ...
        {
            if (metadata.Any(x => string.Equals(x.Key, Constants.GrpcMetaDataKeyError, StringComparison.OrdinalIgnoreCase)))
            {
                string serializedExceptionContent = metadata.SingleOrDefault(x => string.Equals(x.Key, Constants.GrpcMetaDataKeyError, StringComparison.OrdinalIgnoreCase))?.Value;
